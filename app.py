...
# Ruta donde Render descarga los archivos .se1
...
def root():
    return "✔️ Efemérides API activa."
...
def aspect_hits():
    try:
        ...
        bodies   = data["bodies"]                      # ["SATURN", "NEPTUNE"]
        target   = data["target"]                      # puede ser número o string
        aspect   = data.get("aspect", 0)
        orb      = data.get("orb", 0.05)
        jd_start = data["jd_start"]                    # "2025-01-01"
        jd_end   = data["jd_end"]                      # "2025-12-31"
        natal    = data.get("natal_chart", {})         # ← protegido

        jd0 = swe.julday(*map(int, jd_start.split("-")), 0)
        jd1 = swe.julday(*map(int, jd_end.split("-")), 0)

        # Resolver target → grados
        if isinstance(target, (int, float)) or str(target).replace('.', '', 1).isdigit():
            ...
            if target_deg is None:
                return jsonify({
                    "error": f"Target '{target}' no está presente en natal_chart."
                }), 400
